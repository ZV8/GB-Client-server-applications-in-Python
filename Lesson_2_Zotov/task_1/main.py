...
def get_data():
    ...
    for i in range(1, 4):
        file_obj = open(f'info_{i}.txt', mode='rb')
        data_str = file_obj.read()
        result = chardet.detect(data_str)
        str_original = data_str.decode(result['encoding'])
        str_bytes = str_original.encode('utf-8')
        data = str_bytes.decode('utf-8')
